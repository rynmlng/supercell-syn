...
- Runs daily at 13:20 UTC via GitHub Actions (March–June); HRRR 12Z fh=6 publishes at ~13:07 UTC, fh=9 at ~13:12, fh=12 at ~13:17
...
4. Drill into areas of high reflectivity from (3) and pull sounding charts. Look for low-level jets (LLJs) at 850–925 hPa and notable directional & speed shear in the hodograph. Note these areas as the Hatch Area. Focus on daytime convection (fh=3–13, valid 15Z–01Z, through local sunset) — do not anchor on overnight or pre-dawn convection. Do not artificially cap at an earlier hour if the peak setup clearly occurs later in the afternoon or early evening.
   - **Discrete supercell mode:** derive the Hatch Area from sounding and moisture boundary analysis as described. The dryline/moisture boundary is the primary anchor.
   - **QLCS mode:** anchor the Hatch Area latitude to the southernmost contiguous ≥50 dBZ segment of the linear system on the composite reflectivity chart. Do not use the moisture boundary as the latitude anchor — in QLCS setups the active cells ride the frontal boundary, which is typically well north of the dryline. Pull a sounding at that reflectivity-derived location to confirm LLJ and shear, but the reflectivity is the primary placement anchor.
...
6. Draw a point from the Hatch Area in the direction the storm moves TOWARD (BRM direction + 180°). Distance is BRM speed × 7, clamped to 75–250 miles (fast storm ~35 kt → 250 mi, slow storm ~20 kt → 140 mi). This targets roughly 7 hours of lead time regardless of storm speed. That is the Positioning Location.
